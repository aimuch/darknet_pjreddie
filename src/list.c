...
{
	// 输入的node n是链表结构中的第一个(最前面的)node(从free_list函数调用可以看出)，
	// 因此是从前到后释放内存的，在释放每个node的之前，必须首先获取该node的下一个node，否则一旦过早释放，
...
void free_list(list *l)
{
	...
	// 因此需要调用free_node()函数从表头递归释放内存
	free_node(l->front);

	// 释放完node内存之后，在释放l本身中含有的其他数据类型，即int类型
	// 同样，node的释放必须在释放l之前，否则将访问不到node，无法顺利释放node的内存，很可能造成内存泄漏
	free(l);
}


/*
**	释放链表list中的node节点，但是不释放list本身
*/
...
**	返回: 返回类型为void**指针，因此，返回后，需要视具体情况进行指针类型强转，比如转为char**(二维字符数组)
...
void **list_to_array(list *l)
{
    ...
    // 遍历l中所有的节点，将各节点的值取出来放到a中
    while(n){
        // 为每一个字符数组赋值
        // 直接用指针赋值(n->val是指针，a[count++]也是指针，且都是void*型)，不再需要为每一个a[i]动态分配内存，这里极好地使用了指针带来的便利
        a[count++] = n->val;
