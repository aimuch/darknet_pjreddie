...
list *read_data_cfg(char *filename)
{
    ...
    if(file == 0) file_error(filename); // 若文件打开失败, 将直接退出程序, 不会返空指针
...
{
    // 获取l中的头结点(一个节点的值包含了一条项目的信息: 键值与值两部分, 
    ...
    while(n){
        ...
        // 比较该节点的值的键值是否等于指定的键值key, 如果等于0, 则说明找到了, 返回找到的值中(C字符数组指针)
